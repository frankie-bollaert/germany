...
| ID | State | Downloader | `las` (point cloud) | `dgm1` (terrain) | CRS |
|----|-------|-----------|---------------------|------------------|-----|
| **BY** | Bayern | `download_by_lidar.sh` | ✅ ~1 km tiles, multi-TB | ✅ 71,979 tiles, 217 GB | 25832 |
| **BE** | Berlin | `download_be_lidar.sh` | ✅ 9 region packages | ✅ 297 tiles, ~0.2 GB | 25833 |
| **BB** | Brandenburg | `download_bb_lidar.sh` | ✅ 13,086 tiles, ~1.35 TB | ✅ 31,291 tiles, ~36 GB | 25833 |
| **MV** | Mecklenburg-Vorpommern | `download_mv_lidar.sh` | ✅ 25,466 tiles | ✅ 6,407 tiles | 25833 |
| **NI** | Niedersachsen | `download_ni_lidar.sh` | ❌ none published | ✅ 49,708 tiles, **already COG** | 25832 |
| **NW** | Nordrhein-Westfalen | `download_nrw_lidar.sh` † | ✅ 35,860 tiles, 3.49 TB | ✅ 35,860 tiles, 78.8 GB | 25832 |
| **RP** | Rheinland-Pfalz | `download_rlp_lidar.sh` † | ✅ ~21,207 tiles, 5.18 TB | ✅ ~21,082 tiles, 32.8 GB | 25832 |
| **SN** | Sachsen | `download_sn_lidar.sh` | ✅ 4,981 tiles (2 km) | ✅ 4,981 tiles (2 km) | 25833 |
| **BW** | Baden-Württemberg | `download_bw_lidar.sh` | ❌ none published | ✅ 9,370 zips, ~125 GB | 25832 |

**The ID** is the ISO 3166-2 code without `DE-`, and it is the only state identifier used here
— upper case in tables and on the maps, lower case as a CLI argument (`download_alkis.sh nw`),
a `STATES=` value and an output directory. † marks the two script filenames that predate the
rule and spell `nw`/`rp` as `nrw`/`rlp`; see
[The state ID](bundeslaender.md#the-state-id).
